...
/// WMV3 / VC-1 Simple/Main profile decoder.
...
impl Wmv3Decoder {
    pub fn new(width: u32, height: u32, extradata: &[u8]) -> Result<Self> {
        ...
        let mb_dec = MacroblockDecoder::new(width, height);
        let cur = YuvFrame::new(width, height);
        Ok(Self { seq, mb_dec, cur })
    }

    pub fn decode_frame_owned(&mut self, payload: &[u8], pts_ms: u32) -> Result<Option<YuvFrame>> {
        if payload.is_empty() {
            return Ok(None);
        }
        let pic = PictureHeader::parse_simple(payload, &self.seq, pts_ms)?;
        if matches!(pic.frame_type, crate::vc1::FrameType::Skipped) {
            return Ok(None);
        }
        self.mb_dec
            .decode_frame(payload, &pic, &self.seq, &mut self.cur)?;
        Ok(Some(self.cur.clone()))
    }
}

enum AsfVideoDecoder {
    Wmv2(Wmv2Decoder),
    Wmv3(Wmv3Decoder),
}
...
impl<R: Read + Seek> AsfWmv2Decoder<R> {
    ...
    ///
    /// The decoder selects the first video stream whose FourCC is WMV3, WMV2 or WMV1.
    pub fn open(mut reader: R) -> Result<Self> {
        let asf = AsfFile::open(&mut reader)?;
        ...
        let Some(video_info) = video_info else {
            return Err(DecoderError::Unsupported(
                "No WMV3/WMV2/WMV1 video stream found".into(),
            ));
        };
        ...
        let four_cc = std::str::from_utf8(&video_info.codec_four_cc)
            .unwrap_or("")
            .to_uppercase();
        let decoder = match four_cc.as_str() {
            "WMV3" => AsfVideoDecoder::Wmv3(Wmv3Decoder::new(
                video_info.width,
                video_info.height,
                &video_info.extra_data,
            )?),
            _ => AsfVideoDecoder::Wmv2(Wmv2Decoder::new(
                video_info.width,
                video_info.height,
                &video_info.extra_data,
            )),
    ...
    pub fn next_frame(&mut self) -> Result<Option<DecodedFrame>> {
        loop {
            ...
            for payload in payloads {
                ...
                let frame = match &mut self.decoder {
                    AsfVideoDecoder::Wmv2(decoder) => decoder.decode_frame_owned(&data, is_key)?,
                    AsfVideoDecoder::Wmv3(decoder) => decoder.decode_frame_owned(&data, pts_ms)?,
                };
