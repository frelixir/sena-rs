...
final class GameLibrary: ObservableObject {
    ...
    // This must only stop the modal loop; the actual sena entry is called outside SwiftUI.
    var onLaunchRequest: ((GameEntry) -> Void)? = nil

    private let fm = FileManager.default

    private var libraryURL: URL {
        // ~/Library/Application Support/SenaLauncher/library.json
        let appSupport = fm.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        let dir = appSupport.appendingPathComponent("SenaLauncher", isDirectory: true)
    ...
    private func prepareImport(url: URL) {
        let title = senaGameName(for: url)
        let path = url.path
        let id = stableId(for: path)
        let coverPath = senaGameCoverPath(for: url)
    ...
    func refreshValidation() {
        ...
        games.removeAll { g in
            ...
            let ok = fm.fileExists(atPath: g.rootPath) && ((try? root.resourceValues(forKeys: [.isDirectoryKey]).isDirectory) ?? false)
            if !ok { changed = true }
            return !ok
        }
        if changed { save() }
