...
private func redirectStdoutStderrToLogFile() {
    ...
    // Mirror logs to ~/Library/Logs/SenaLauncher/console.log for debugging.
    let fm = FileManager.default
    guard let library = fm.urls(for: .libraryDirectory, in: .userDomainMask).first else { return }
    let dir = library.appendingPathComponent("Logs/SenaLauncher", isDirectory: true)
...
@_silgen_name("sena_pump_submit_messagebox_result")
private func sena_pump_submit_messagebox_result(_ handle: UnsafeMutableRawPointer?, _ requestId: UInt64, _ value: Int64) -> Void

@_silgen_name("sena_pump_step")
private func sena_pump_step(_ handle: UnsafeMutableRawPointer?, _ timeoutMs: UInt32) -> Int32

@_silgen_name("sena_pump_destroy")
private func sena_pump_destroy(_ handle: UnsafeMutableRawPointer?) -> Void
...
private func runMessagebox(kind: Int32, title: String, message: String) -> Int64 {
    ...
    alert.messageText = title.isEmpty ? "Sena" : title
...
@_cdecl("sena_macos_messagebox_callback")
func sena_macos_messagebox_callback(
