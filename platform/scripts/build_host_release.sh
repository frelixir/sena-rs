...
CRATE_PKG="${CRATE_PKG:-pal-vm}" # cargo package name (-p)
BIN_NAME="${BIN_NAME:-sena}"     # cargo binary/output name
...
build_with_cargo_target() {
  ...
  log "cargo build --${PROFILE} -p ${CRATE_PKG} --bin ${BIN_NAME} --target ${tgt}"
  (cd "${ROOT_DIR}" && cargo build --"${PROFILE}" -p "${CRATE_PKG}" --bin "${BIN_NAME}" --target "${tgt}")
}

build_with_cargo_host() {
  log "cargo build --${PROFILE} -p ${CRATE_PKG} --bin ${BIN_NAME}"
  (cd "${ROOT_DIR}" && cargo build --"${PROFILE}" -p "${CRATE_PKG}" --bin "${BIN_NAME}")
}

build_with_cross_target() {
  local tgt="$1"
  need_cmd cross
  log "cross build --${PROFILE} -p ${CRATE_PKG} --bin ${BIN_NAME} --target ${tgt}"
  (cd "${ROOT_DIR}" && cross build --"${PROFILE}" -p "${CRATE_PKG}" --bin "${BIN_NAME}" --target "${tgt}")
}
