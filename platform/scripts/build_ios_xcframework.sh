...
OUT_XCF="$VENDOR_DIR/Sena.xcframework"

SENA_CARGO_PKG="${SENA_CARGO_PKG:-pal-vm}"
RUST_LIB_NAME="${RUST_LIB_NAME:-pal_vm}" # cargo output: lib${RUST_LIB_NAME}.a
LIB_NAME="${LIB_NAME:-sena}" # xcframework public library name: lib${LIB_NAME}.a
...
[[ -f "$HDR_DIR/sena.h" ]] || { echo "ERROR: Missing header: $HDR_DIR/sena.h" >&2; exit 1; }
...
(cd "$ROOT_DIR" && cargo build --release -p "$SENA_CARGO_PKG" --lib --target "$TGT_IOS")
(cd "$ROOT_DIR" && cargo build --release -p "$SENA_CARGO_PKG" --lib --target "$TGT_SIM")
...
TMP_LIB_DIR="$VENDOR_DIR/.sena-xcframework-input"
...
if [[ ! -f "$LIB_IOS_RUST_A" ]]; then
  ...
  echo "Hint: ensure pal-vm outputs staticlib for iOS." >&2
  exit 1
fi
