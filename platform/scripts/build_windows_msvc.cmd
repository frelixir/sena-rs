...
rem Build sena for Windows using MSVC targets.
...
rem   CRATE_PKG=pal-vm  (cargo package name)
rem   BIN_NAME=sena      (cargo binary name)
rem   APP_NAME=sena             (output exe name)
rem --------------------------------------------
...
if "%CRATE_PKG%"=="" set "CRATE_PKG=pal-vm"
if "%BIN_NAME%"=="" set "BIN_NAME=sena"
if "%APP_NAME%"=="" set "APP_NAME=sena"
...
pushd "%ROOT_DIR%" >nul
cargo build --release -p "%CRATE_PKG%" --bin "%BIN_NAME%" --target "%TARGET_TRIPLE%"
if errorlevel 1 (
  popd >nul
...
set "EXE_IN=%ROOT_DIR%\target\%TARGET_TRIPLE%\release\%BIN_NAME%.exe"
if not exist "%EXE_IN%" (
...
echo   set CRATE_PKG=pal-vm
echo   set APP_NAME=sena
exit /b 2
