...
SENA_RUST_LIB_NAME="${SENA_RUST_LIB_NAME:-pal_vm}"
SENA_SO_NAME="${SENA_SO_NAME:-sena}"
...
cargo ndk $(for abi in ${ABIS}; do printf -- "-t %s " "${abi}"; done) \
  -o "${JNI_LIBS_DIR}" \
  build ${CARGO_PROFILE_ARGS[@]+"${CARGO_PROFILE_ARGS[@]}"} -p "${SENA_CARGO_PKG}" --lib
popd >/dev/null

# Cargo outputs lib${SENA_RUST_LIB_NAME}.so for the pal-vm package.
# The Android Java side intentionally loads the stable ABI name libsena.so.
for abi in ${ABIS}; do
  src="${JNI_LIBS_DIR}/${abi}/lib${SENA_RUST_LIB_NAME}.so"
  dst="${JNI_LIBS_DIR}/${abi}/lib${SENA_SO_NAME}.so"
  if [[ -f "${src}" ]]; then
    cp -f "${src}" "${dst}"
  fi
done
...
missing=0
for abi in ${ABIS}; do
  so_path="${JNI_LIBS_DIR}/${abi}/lib${SENA_SO_NAME}.so"
  if [[ ! -f "${so_path}" ]]; then
...
SENA_ABIS_PROP="$(printf "%s" "${ABIS}" | tr " " ",")"
if [[ -x "./gradlew" ]]; then
  GRADLE_CMD="./gradlew"
...
echo "[android] Running Gradle: ${GRADLE_CMD} -PsenaAbis=${SENA_ABIS_PROP} ${GRADLE_TASK}"
${GRADLE_CMD} -PsenaAbis="${SENA_ABIS_PROP}" ${GRADLE_TASK}
