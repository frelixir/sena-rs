...
if [[ ! -d "$VENDOR" ]]; then
  echo "[ios] Missing $VENDOR, building Sena.xcframework..."
  "$SCRIPT_DIR/build_ios_xcframework.sh"
fi

[[ -d "$VENDOR" ]] || { echo "ERROR: Sena.xcframework still missing after build: $VENDOR" >&2; exit 1; }
...
APP_PATH="$DERIVED/Build/Products/$CONFIG-iphoneos/Sena.app"
if [[ ! -d "$APP_PATH" ]]; then
...
IPA_OUT="$DIST_DIR/SenaLauncher.ipa"
rm -f "$IPA_OUT"
